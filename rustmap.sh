...
echo "=== Building RustMap ==="
cargo build --release

echo -e "\n=== Basic Local Scan ==="
cargo run -- -a 127.0.0.1

cargo run -- -a 127.0.0.1 -r 1-500 -b 1000

echo -e "\n=== Greppable Output ==="
cargo run -- -a 127.0.0.1 -r 1-100 -g



#DNS output
sudo cargo run -- --udp -a 192.168.31.210 -p 53,67,68,69,123,161 -b 500 -t 5000
...
time cargo run -- -a 127.0.0.1 -p --ulimit 10000 --timeout 150 --greppable
